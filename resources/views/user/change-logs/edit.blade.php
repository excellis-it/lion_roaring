        ...
        <div class="bg_white_border">
            <div class="row mb-3 align-items-center">
                ...
                <div class="col-md-4 text-end">
                    <a href="{{ route('change-logs.index', ['platform' => $changeLog->platform]) }}"
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="type" class="form-control @error('type') is-invalid @enderror">
                            @foreach (['feature', 'improvement', 'bugfix', 'security'] as $type)
                                <option value="{{ $type }}" @selected(old('type', $changeLog->type) === $type)>{{ ucfirst($type) }}
                                </option>
                            @endforeach
                        </select>
                        @error('type')
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea name="description" id="description" rows="10"
                            class="form-control notranslate @error('description') is-invalid @enderror" translate="no">{{ old('description', $changeLog->description) }}</textarea>
...
@push('scripts')
    ...
    <style>
        #changeLogForm .note-editable ul {
            list-style-type: disc;
            padding-left: 1.5rem;
            margin: 0.35rem 0;
        }

        #changeLogForm .note-editable ol {
            list-style-type: decimal;
            padding-left: 1.5rem;
            margin: 0.35rem 0;
        }

        #changeLogForm .note-editable li {
    ...
    <script>
        $(function() {
            ...
            $('#changeLogForm').on('submit', function(e) {
                ...
                setPublishMax();
                if ($publishedAt.val() && $publishedAt.attr('max') && $publishedAt.val() > $publishedAt
                    .attr('max')) {
                    e.preventDefault();
