                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    @foreach ($role->permissions as $permission)
                                                                        <span
                                                                            class="round-btn">{{ $permission->name }}</span>
                                                                    @endforeach
                                                                </td>
                                                                <td>
                                                                    <div class="d-flex">
                                                                        <a href="{{route('roles.edit', Crypt::encrypt($role->id))}}" class="edit_icon me-2">
                                                                            <i class="ti ti-edit"></i>
                                                                        </a>
                                                                        @if ($role->name == 'CUSTOMER' || $role->name == 'LEADER')
                                                                        @else

                                                                        <a href="javascript:void(0);" data-route="{{ route('roles.delete', Crypt::encrypt($role->id)) }}" class="delete_icon" id="delete">
