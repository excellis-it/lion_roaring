...
@foreach ($organization_centers as $key => $organization_center)
    <tr>
        <td>{{ ($organization_centers->currentPage()-1) * $organization_centers->perPage() + $loop->index + 1 }}</td>
        <td>{{ ($organization_center->ourOrganization->name) ? $organization_center->ourOrganization->name : 'N/A' }}</td>
        ...
        <td>
            <div class="edit-1 d-flex align-items-center justify-content-center">
                ...
                <a title="Delete" data-route="{{ route('organization-centers.delete', $organization_center->id) }}"
