...
class EcclessiaController extends Controller
{
    public function index()
    {
        if (auth()->user()->can('Manage All Users')) {
            ...
            return view('user.admin.ecclessia.list')->with(compact('ecclessias', 'countries', 'roles'));
        } else {
    ...
    public function edit($id)
    {
        if (auth()->user()->can('Edit All Users')) {
            ...
            $roles = Role::where('name', '!=', 'SUPER ADMIN')->whereIn('type', [1, 3])->get();
            $edit = true;
            return response()->json(['data' => view('user.admin.ecclessia.edit', compact('eclessia', 'countries', 'edit', 'roles'))->render()]);
        } else {
            abort(403, 'You do not have permission to access this page.');
        }
    }
