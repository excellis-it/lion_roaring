...
- **Media on first save:** when creating that new country row without re-uploading files, image/video/PDF path fields are copied from the US row via `Helper::applyUsCmsMediaDefaults()` (and related gallery rows for Organization / Principle & Business). Storage paths are shared — US files are not deleted or duplicated on disk. E-Store / E-Learning home CMS use `Helper::seedNewCmsRowFromUs()` for the same purpose (including slider JSON images).
